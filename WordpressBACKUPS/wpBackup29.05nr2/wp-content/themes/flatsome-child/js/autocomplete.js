...
function getMap(){
  ...
  selectedRoom = document.getElementById("rooms").value.toLowerCase();
  //find the associated floor map for the selected room
  for(i = 0; i < roomList.length; i++){
    if (roomList[i].Room.toLowerCase()==selectedRoom){
      floorName=roomList[i].Map;
      selectedRoom=roomList[i].Room;
    }
  }
  var destinationDiv = document.getElementById("map");
  //delete the map from previous searches
  while (destinationDiv.firstChild) {
      destinationDiv.removeChild(destinationDiv.firstChild);
      }
  //create a new image element, append it to the div in order to display the map
  var floorImage = document.createElement("IMG");
  floorImage.setAttribute("src", "https://tek.westerdals.no/~breale17/wp/wp-content/themes/flatsome-child/images/plans/" + floorName + "_" + selectedRoom + ".png");
...
var roomList = [
  ...
  {Room:"Bibliotek",Map:"floor1"},
  {Room:"Auditorium",Map:"floor1"},
  {Room:"Resepsjon",Map:"floor1"},
  {Room:"Sjenkestua",Map:"floor1"},
  {Room:"Kantine",Map:"floor1"},
...
document.getElementById("button").addEventListener('click', getMap);
