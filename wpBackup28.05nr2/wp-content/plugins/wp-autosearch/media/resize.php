...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
    function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
        ...
        global $wpdb, $wizardinfosys_autosearch;

        if ( empty( $url ) )
            return new WP_Error( 'no_image_url', __( 'No image URL has been entered.', $wizardinfosys_autosearch->text_domain ), $url );
	...
	$retina = $retina ? ( $retina === true ? 2 : $retina ) : 1;
      ...
      // Get image file path
		...
		$file_path=$upload_dir['path'].'/'.$infoUrl['basename'];
